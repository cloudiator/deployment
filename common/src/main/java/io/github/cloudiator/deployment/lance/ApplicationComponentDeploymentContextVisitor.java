...
import de.uniulm.omi.cloudiator.domain.PortProvided;
import de.uniulm.omi.cloudiator.domain.PortRequired;
...
public class ApplicationComponentDeploymentContextVisitor {
    ...
    public DeploymentContext registerAtDeploymentContext(DeploymentContext deploymentContext) {
        ...
        for (PortProvided portProvided : applicationComponent.providedPorts()) {
            deploymentContext
                .setProperty(portProvided.name(), portProvided.port(), InPort.class);
        }
        for (PortRequired portRequired : applicationComponent.requiredPorts()) {
